...
DESCRIPTION       : Identifica si el colaborador es New Hire, usando la misma
                    ...
                    v14: se agrega comparacion de PER_ASG_LEGAL_ENTITY entre
                    el assignment anterior encontrado y el assignment actual.
                    Si la entidad legal cambia, se considera movimiento
                    intercompania sin depender del reason code capturado
                    (dato estructural, no requiere catalogo mantenido por
                    equipo funcional). Si la entidad legal es la misma, se
                    conserva el chequeo de reason codes confirmados (28,
                    C28, 00, U00, 115, 64, 114) como respaldo.
**********************************************************************/
...
(
    ...
    (
        ...
        AND L_ASG_ID_ANTERIOR = 0
         THEN
        (
            CHANGE_CONTEXTS(HR_ASSIGNMENT_ID = L_FILA_ASG_ID, 
            EFFECTIVE_DATE = L_FILA_FIN)
            (
            ...
            l_log = SET_LOG('Candidato assignment ' 
            || TO_CHAR(L_FILA_ASG_ID) 
            || ' fecha fin ' || TO_CHAR(L_FILA_FIN, 'YYYY/MM/DD') 
            || ' action code: ' || L_ACTION_CODE_TEMP 
            || ' reason code: ' || L_REASON_CODE_TEMP 
            || ' legal entity: ' || TO_CHAR(L_LEGAL_ENTITY_TEMP))
    ...
    (
        ...
        (
            ...
            (
                L_NEW_HIRE = 'N'
                l_log = SET_LOG('No es New Hire: misma entidad legal, pero reason code de movimiento de exclusion detectado (' || L_REASON_CODE || ')')
            )
            ELSE
            (
                L_NEW_HIRE = 'Y'
                l_log = SET_LOG('Es New Hire: misma entidad legal y reason code no corresponde a movimiento interno conocido (' || L_REASON_CODE || ')')
            )
        )
    )
    ELSE
